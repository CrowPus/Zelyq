-- 058: the Supabase connection became instance-wide (managed from Settings),
-- so `provider_connections` no longer carries `team_id`. The four provider
-- tables added in 0009 hold no data yet (no UI wrote to them before this),
-- so this rebuilds them in the new shape rather than doing a fragile
-- SQLite column-drop. Nothing outside these four tables is touched.
DROP TABLE IF EXISTS `provider_operations`;--> statement-breakpoint
DROP TABLE IF EXISTS `project_provider_links`;--> statement-breakpoint
DROP TABLE IF EXISTS `provider_resources`;--> statement-breakpoint
DROP TABLE IF EXISTS `provider_connections`;--> statement-breakpoint
