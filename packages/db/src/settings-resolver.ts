 ...
 * the server owns.
 *
 * Before this, `SettingsService` (in `apps/server`) had its own copy of
 * this rule and nothing else did, which is how a setting could be stored
 * correctly in the database and still
 ...
 * whatever the environment says is not actually protecting anything. Every
 * caller of this function already requires an instance admin to read or
 * write the value, so "an operator set this deliberately" and
