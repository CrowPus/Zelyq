...
| `ZELYQ_TRUST_PROXY` | off | Believe `X-Forwarded-*`. Set it when Zelyq runs behind a reverse proxy or load balancer: `true` for the usual single proxy, a number for that many hops, or a comma-separated list of proxy addresses/subnets to trust. It is what makes the session cookie `Secure` on an HTTPS deployment (the cookie follows the scheme of the request the server sees) and what makes logged client IPs the real ones. Leave it off when Zelyq is exposed directly — otherwise any caller can claim any address or scheme by sending a header. |
...
| `ZELYQ_CONTAINER_IMAGE` | `zelyq/sandbox:node22-python311` (built on first use) | Image agent commands run in, for `container`. The default is `node:22-bookworm-slim` plus git, ca-certificates and ssh — see `docker/sandbox.Dockerfile`. Any image you name here must contain git, or per-turn commits and pushes cannot work. |
...
| `ZELYQ_PREVIEW_URL_TEMPLATE` | — | Optional. A template with a `{port}` placeholder (e.g. `https://p{port}.preview.example.com`). When set, preview URLs are built from it instead of `http://<host>:<port>` — for a deployment that reverse-proxies each preview over one HTTPS origin rather than exposing the `4300-4399` range. It applies to all three runtimes, including `remote`, where it is read on the side that serves the browser rather than on the runtime host, so a host operator has no second copy to keep in step. Needs a wildcard DNS record, a wildcard TLS cert, and an nginx vhost that maps the subdomain back to `127.0.0.1:<port>`. See [self-hosting.md](./self-hosting.md#4-previews-behind-the-proxy). |
...
| `ZELYQ_IMAGE_GENERATION_PROVIDER` | `openai` | Which provider Image Studio generates with: `openai`, `google`, or `xai`. Deliberately separate from `ZELYQ_IMAGE_PROVIDER` above, which is stock-photo *retrieval* — these are different jobs and different accounts. |
| `ZELYQ_IMAGE_API_KEY` | — | OpenAI API key with image-model access, for Image Studio. Billed to this key; a chat subscription does not cover it. Stored encrypted when set in Settings instead. |
| `ZELYQ_IMAGE_MODEL` | `gpt-image-2` | The OpenAI image model. |
| `ZELYQ_IMAGE_GOOGLE_API_KEY` / `ZELYQ_IMAGE_GOOGLE_MODEL` | — / `gemini-3.1-flash-image` | The same for Google (Nano Banana). Supports reference images. |
| `ZELYQ_IMAGE_XAI_API_KEY` / `ZELYQ_IMAGE_XAI_MODEL` | — / `grok-imagine-image-2.0` | The same for xAI. Prompt-only: this version has no confirmed reference-image contract for xAI, so references are rejected at submit. |
| `ZELYQ_IMAGE_ASSETS_DIR` | `images/` beside the database | Where generated images and their reference uploads are written. Private authenticated files, not public static assets. Back it up **with** the database and the encryption key; every API replica sharing a database must share this directory. |
| `ZELYQ_IMAGE_HOURLY_LIMIT` | `30` | Images one user may request per hour, counting Image Studio and the build agent together. This is the spending limit — each image is billed to the key above. |
| `ZELYQ_IMAGE_SESSION_LIMIT` | `6` | Images the build agent may generate in one conversation. Bounds a retry loop, or an instruction hidden in a cloned repository, so it cannot spend an hour's budget in a single turn. Agent generation is also **off per project** until someone turns it on. |
| `ZELYQ_VIDEO_GENERATION_PROVIDER` | `google` | Which provider Video Studio generates with: `google` (Veo) or `xai` (Grok Imagine). Separate from the image settings above — video is a different model, a different credential and a very different price per call. |
| `ZELYQ_VIDEO_GOOGLE_API_KEY` / `ZELYQ_VIDEO_GOOGLE_MODEL` | — / `veo-3.1-generate-preview` | Google video credential and model. There is no fallback to the image key: a video credential must be set explicitly, because it spends far more per request. |
| `ZELYQ_VIDEO_XAI_API_KEY` / `ZELYQ_VIDEO_XAI_MODEL` | — / first catalogued model | The same for xAI. |
| `ZELYQ_VIDEO_ASSETS_DIR` | `videos/` beside the database | Where generated clips and their starting images are written. Private authenticated files, streamed with range support — never a public static directory. Back it up **with** the database and the encryption key; API replicas sharing a database must share it. Budget for it: a clip is capped at 200 MiB and a user's library at 5 GiB. |
| `ZELYQ_VIDEO_HOURLY_LIMIT` | `5` | Clips one user may submit per rolling hour, counting requests whose outcome was never confirmed — because those may still have been charged. This is the spending limit. |
| `ZELYQ_VIDEO_CONCURRENCY` | `2` | Outstanding provider jobs across the instance (1–10). Deliberately separate from image capacity, so a slow video cannot occupy an image slot. |
| `ZELYQ_VIDEO_SESSION_LIMIT` | `2` | Clips the build agent may generate in one conversation. Far lower than the image equivalent because a clip costs much more; a hero needs one, not six. Agent video is additionally **off per project** until switched on, and that permission is separate from the image one. |
