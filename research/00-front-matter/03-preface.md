...
For Zelyq, the book is also the gate before code. A contributor should not begin implementation merely because an idea is appealing or technically possible. The problem must be recorded, relevant research must be assessed, alternatives and consequences must be reviewed, and the exact work must be authorized in a project-specific engineering entry. That decision record remains separate from the research finding so the product cannot rewrite the evidence to justify itself.
